...
The confusion matrix for the Decision Tree semi-supervised learning model shows that the classifier correctly identified 37 metaphorical excerpts (True Positives) and 44 non-metaphorical excerpts (True Negatives). However, the model also produced 9 False Positives and 12 False Negatives, indicating some difficulty distinguishing subtle metaphorical expressions from literal language.

The relatively higher False Negative count suggests that the Decision Tree model struggled to capture certain complex metaphorical patterns within the unlabeled augmented dataset. This limitation may be due to the model’s tendency to create rigid decision boundaries, making it less effective for handling nuanced semantic relationships commonly found in literary metaphors.

![Support Vector Machine Confusion Matrix- SSL Approach](images/SVM_ssl_Matrix.PNG)

The confusion matrix for the SSL-based SVM model shows balanced but comparatively weaker performance than its fully supervised counterpart. The model correctly classified 40 metaphorical excerpts (True Positives) and 42 non-metaphorical excerpts (True Negatives), while producing 11 False Positives and 9 False Negatives.

The increased misclassification rate suggests that the pseudo-labeled data introduced some noisy semantic boundaries during iterative training. Since SVM attempts to maximize class separation margins, incorrectly pseudo-labeled samples may have reduced the model’s ability to construct optimal decision boundaries for metaphor classification.
